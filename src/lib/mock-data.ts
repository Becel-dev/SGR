import type { Risk, RecentActivity, BowtieData } from './types';

export const kpiData = {
  compliance: {
    value: 92.5,
    change: 1.2,
  },
  overdueItems: {
    value: 8,
    change: -3,
  },
  criticalRisks: {
    value: 3,
    change: 1,
  },
  controlDeviations: {
    value: 5,
    change: 2,
  },
};

export const complianceChartData = [
  { month: 'Jan', compliance: 88, target: 90 },
  { month: 'Feb', compliance: 91, target: 90 },
  { month: 'Mar', compliance: 92, target: 90 },
  { month: 'Apr', compliance: 89, target: 90 },
  { month: 'May', compliance: 93, target: 90 },
  { month: 'Jun', compliance: 95, target: 90 },
  { month: 'Jul', compliance: 92.5, target: 90 },
];

export const statusBreakdownChartData = [
  { name: 'No Prazo', value: 400, fill: 'hsl(var(--chart-1))' },
  { name: 'Em Risco', value: 150, fill: 'hsl(var(--chart-2))' },
  { name: 'Atrasado', value: 50, fill: 'hsl(var(--destructive))' },
];

export const riskMatrixData = [
  { impact: 'Insignificante', probability: 0, color: 'bg-green-200' },
  { impact: 'Menor', probability: 1, color: 'bg-yellow-200' },
  { impact: 'Moderado', probability: 2, color: 'bg-orange-300' },
  { impact: 'Maior', probability: 3, color: 'bg-red-400' },
  { impact: 'Catastrófico', probability: 4, color: 'bg-red-600' },
];


export const recentActivityData: RecentActivity[] = [
    {
        id: '1',
        user: { name: 'Alice Johnson', avatarUrl: 'https://picsum.photos/seed/101/32/32' },
        action: 'criou um novo risco',
        target: 'Falha no sistema de freios',
        timestamp: '2 horas atrás',
    },
    {
        id: '2',
        user: { name: 'Bob Williams', avatarUrl: 'https://picsum.photos/seed/102/32/32' },
        action: 'atualizou o controle',
        target: 'Inspeção de via semanal',
        timestamp: '5 horas atrás',
    },
    {
        id: '3',
        user: { name: 'Carla Davis', avatarUrl: 'https://picsum.photos/seed/103/32/32' },
        action: 'aprovou o risco',
        target: 'Descarrilamento em pátio',
        timestamp: '1 dia atrás',
    },
    {
        id: '4',
        user: { name: 'David Miller', avatarUrl: 'https://picsum.photos/seed/104/32/32' },
        action: 'comentou no KPI',
        target: '% de conformidade de inspeção',
        timestamp: '2 dias atrás',
    },
];

export const risksData: Risk[] = [
  {
    id: 'R001',
    gerencia: 'Operação',
    risco: 'Falha no sistema de freios',
    descricaoDoRisco: 'Falha completa ou parcial do sistema de frenagem do trem, resultando em incapacidade de parar ou reduzir a velocidade conforme necessário.',
    dataDeIdentificacao: '2023-01-15',
    origemDoRisco: 'Interna',
    causaRaizDoRisco: 'Desgaste excessivo das sapatas de freio',
    consequenciaDoRisco: 'Colisão com outros trens ou obstáculos, descarrilamento.',
    tipoDeRisco: 'Operacional',
    processoAfetado: 'Operação Ferroviária',
    probabilidadeInerente: 'Possível',
    impactoInerente: 'Catastrófico',
    nivelDeRiscoInerente: 'Crítico',
    estrategia: 'Mitigar',
    descricaoDoControle: 'Inspeções regulares e substituição preventiva das sapatas de freio.',
    naturezaDoControle: 'Manual',
    tipoDeControle: 'Preventivo',
    classificacaoDoControle: 'Chave',
    frequencia: 'Semanal',
    eficaciaDoControle: 'Eficaz',
    probabilidadeResidual: 'Raro',
    impactoResidual: 'Maior',
    nivelDeRiscoResidual: 'Alto',
    statusDoRisco: 'Em Tratamento',
    responsavelPeloRisco: 'João Silva',
    dataDaUltimaRevisao: '2024-07-15',
    dataDaProximaRevisao: '2025-01-15',
  },
  {
    id: 'R002',
    gerencia: 'Operação',
    risco: 'Descarrilamento em pátio',
    descricaoDoRisco: 'Saída do trem dos trilhos durante manobras em pátios de baixa velocidade.',
    dataDeIdentificacao: '2023-02-20',
    origemDoRisco: 'Interna',
    causaRaizDoRisco: 'AMV (Aparelho de Mudança de Via) com defeito',
    consequenciaDoRisco: 'Danos ao material rodante e à via, interrupção das operações no pátio.',
    tipoDeRisco: 'Operacional',
    processoAfetado: 'Manobras em Pátio',
    probabilidadeInerente: 'Provável',
    impactoInerente: 'Moderado',
    nivelDeRiscoInerente: 'Alto',
    estrategia: 'Mitigar',
    descricaoDoControle: 'Manutenção preventiva dos AMVs e treinamento dos operadores de manobra.',
    naturezaDoControle: 'Manual',
    tipoDeControle: 'Preventivo',
    classificacaoDoControle: 'Chave',
    frequencia: 'Mensal',
    eficaciaDoControle: 'Eficaz',
    probabilidadeResidual: 'Improvável',
    impactoResidual: 'Menor',
    nivelDeRiscoResidual: 'Baixo',
    statusDoRisco: 'Mitigado',
    responsavelPeloRisco: 'Maria Oliveira',
    dataDaUltimaRevisao: '2024-06-30',
    dataDaProximaRevisao: '2024-12-30',
  },
  {
    id: 'R003',
    gerencia: 'Comercial',
    risco: 'Aumento inesperado do preço do combustível',
    descricaoDoRisco: 'Aumento súbito e significativo no custo do diesel, impactando os custos operacionais.',
    dataDeIdentificacao: '2023-03-10',
    origemDoRisco: 'Externa',
    causaRaizDoRisco: 'Volatilidade do mercado internacional de petróleo.',
    consequenciaDoRisco: 'Aumento dos custos operacionais, redução da margem de lucro.',
    tipoDeRisco: 'Financeiro',
    processoAfetado: 'Gestão de Custos',
    probabilidadeInerente: 'Possível',
    impactoInerente: 'Maior',
    nivelDeRiscoInerente: 'Alto',
    estrategia: 'Transferir',
    descricaoDoControle: 'Contratos de hedge para fixar o preço do combustível.',
    naturezaDoControle: 'Sistêmico',
    tipoDeControle: 'Corretivo',
    classificacaoDoControle: 'Chave',
    frequencia: 'Trimestral',
    eficaciaDoControle: 'Muito Eficaz',
    probabilidadeResidual: 'Improvável',
    impactoResidual: 'Menor',
    nivelDeRiscoResidual: 'Baixo',
    statusDoRisco: 'Fechado',
    responsavelPeloRisco: 'Carlos Pereira',
    dataDaUltimaRevisao: '2024-07-20',
    dataDaProximaRevisao: '2025-01-20',
  },
  {
    id: 'R004',
    gerencia: 'Compliance',
    risco: 'Não conformidade com novas regulamentações ambientais',
    descricaoDoRisco: 'Falha em atender aos novos requisitos regulatórios ambientais, resultando em multas e sanções.',
    dataDeIdentificacao: '2023-04-05',
    origemDoRisco: 'Externa',
    causaRaizDoRisco: 'Falta de monitoramento de mudanças na legislação.',
    consequenciaDoRisco: 'Multas, sanções legais, dano à reputação da empresa.',
    tipoDeRisco: 'Regulatório',
    processoAfetado: 'Gestão Ambiental',
    probabilidadeInerente: 'Improvável',
    impactoInerente: 'Maior',
    nivelDeRiscoInerente: 'Médio',
    estrategia: 'Evitar',
    descricaoDoControle: 'Sistema de monitoramento regulatório e auditorias internas de conformidade.',
    naturezaDoControle: 'Sistêmico',
    tipoDeControle: 'Preventivo',
    classificacaoDoControle: 'Chave',
    frequencia: 'Anual',
    eficaciaDoControle: 'Eficaz',
    probabilidadeResidual: 'Raro',
    impactoResidual: 'Moderado',
    nivelDeRiscoResidual: 'Baixo',
    statusDoRisco: 'Aberto',
    responsavelPeloRisco: 'Ana Souza',
    dataDaUltimaRevisao: '2024-07-01',
    dataDaProximaRevisao: '2025-01-01',
  },
];


export const bowtieData: BowtieData = {
    event: {
        label: "Descarrilamento",
        description: "Saída do trem dos trilhos na via principal.",
    },
    threats: [
        { label: "Excesso de Velocidade", description: "Operação acima da VMA do trecho." },
        { label: "Falha de Material Rodante", description: "Ex: rolamento, truque, rodeiro." },
        { label: "Defeito na Via", description: "Ex: trilho quebrado, fixação." },
    ],
    preventiveControls: [
        { label: "ATC/ATS", description: "Controle de velocidade embarcado." },
        { label: "Manutenção Preditiva", description: "Inspeção e troca de componentes." },
        { label: "Inspeção de Via", description: "Ultrassom, veículo de controle." },
    ],
    consequences: [
        { label: "Danos à Carga", description: "Avaria ou perda total da carga." },
        { label: "Impacto Ambiental", description: "Vazamento de produtos perigosos." },
        { label: "Interdição da Via", description: "Bloqueio da linha para reparos." },
    ],
    mitigatoryControls: [
        { label: "Plano de Emergência", description: "Ação rápida para contenção de danos." },
        { label: "Kits de Mitigação", description: "Materiais para contenção de vazamentos." },
        { label: "Equipe de Restabelecimento", description: "Equipe para liberar a via rapidamente." },
    ]
};
